  ...
  create() {
    ...
    this.socket.on('contagio', (id) => {
        if (id == this.socket.id) {
            this.contagio(this.player, this.obstaculos)
        }
    })
  ...
  update(time, dt) {
    ...
    if (!this.gameOver) {
      if (this.cursors.left.isDown) {
        this.socket.emit("izquierda");
      } else if (this.cursors.right.isDown) {
        this.socket.emit("derecha");
      } else {
        this.socket.emit("quieto");
      }
      if (this.cursors.up.isDown && this.player.body.touching.down) {
        this.socket.emit('arriba');
      }
    }
  }
}
